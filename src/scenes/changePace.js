...
export class ChangePaceScene extends BaseScene {
  constructor(context) {
    super(context);
    this.menu = null;
  }
  ...
  mount() {
    const prompts = this.getPrompt('change_pace');
    if (!prompts) throw new Error('Missing change_pace prompts.');
  ...
  handleSelect(option) {
    this.game.setPace(option.id);
    this.game.addLogEntry({
      type: 'info',
      message: `Pace set to ${option.label.toLowerCase()}.`,
    });
    const returnTo = this.props?.returnTo ?? 'travel';
    this.navigate(returnTo, { source: 'change_pace' });
  }
